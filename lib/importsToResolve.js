...
function importsToResolve(url, extensions) {
  ...
  if (ext === '.scss' || ext === '.sass') {
    return extensions.reduce((acc, extension) => {
      [request, url].forEach((filePath) => {
        const extensionToReplace = filePath.replace(/\.\w+$/, extension);
        acc.push(extensionToReplace);
      });
      return acc;
    }, []);
  }
  ...
  if (basename.charAt(0) === '_') {
    const paths = extensions.map((localExt) => `${request}${localExt}`) || [];
  ...
  extensions.forEach((localExt) =>
    paths.push(`${dirname}/_${basename}${localExt}`)
  );
  extensions.forEach((localExt) => paths.push(`${request}${localExt}`));
  paths.push(url);
